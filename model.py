...
class cyclegan(object):
    def __init__(self, sess, args):
        ...
        self._build_kong_model_D_no_concat()
    ...
    def _build_kong_model_D_no_concat(self):
        ####################################################################################################################################
        ### Generator
        self.curved_concat_straight = tf.placeholder(tf.float32,[None, None, None, self.input_c_dim + self.output_c_dim],name = "curved_concat_straight")
        self.curved   = self.curved_concat_straight[:,:,:,                 :self.input_c_dim ]
        ...
        # self.gen_pair    = tf.concat([self.curved, self.curved_to_straight],3)
        # self.gen_pair_score   = self.discriminator(self.gen_pair, self.options, reuse=False,  name="discriminator")
        self.gen_score   = self.discriminator(self.curved_to_straight, self.options, reuse=False,  name="discriminator")
        self.g_adv_loss  = self.criterionGAN(self.gen_score, tf.ones_like(self.gen_score))
        self.g_loss      = self.g_adv_loss + self.L1_lambda * self.g_mse_loss
        ####################################################################################################################################
        ### Discriminator
        # self.fake_pair       = tf.placeholder(tf.float32, [None, None, None, self.input_c_dim + self.input_c_dim], name = "fake_input_pair")
        self.fake_img       = tf.placeholder(tf.float32, [None, None, None, self.input_c_dim ], name = "fake_input_pair")
        # self.fake_pair_score = self.discriminator(self.fake_pair,        self.options, reuse=True,   name="discriminator")
        self.fake_score = self.discriminator(self.fake_img, self.options, reuse=True,   name="discriminator")
        self.real_score = self.discriminator(self.straight, self.options, reuse=True,   name="discriminator")
        
        self.d_loss_real = self.criterionGAN(self.real_score, tf.ones_like( self.real_score)) 
        self.d_loss_fake = self.criterionGAN(self.fake_score, tf.zeros_like(self.fake_score)) 
        self.d_loss = (self.d_loss_real + self.d_loss_fake)/2
        ########################################################################################################
        ### Tensorboard
        self.g_adv_loss_sum         = tf.summary.scalar("1_g_adv_loss", self.g_adv_loss)
...
##################################################################################################################################################################################################################################
    ...
    def train_kong_D_no_concat(self, args):
        print("train start count time")
        train_start_time = time.time()
...
##################################################################################################################################################################################################################################
                ...
                # Update G network and record fake outputs
                # fake_curved_to_straight, _, summary_str = self.sess.run(
                fake_img, _, summary_str = self.sess.run(
                    [self.curved_to_straight, self.g_optim, self.g_sum],  #[self.fake_A, self.fake_B, self.g_optim, self.g_sum],
                    feed_dict={self.curved_concat_straight: batch_images, self.lr: lr})
                self.writer.add_summary(summary_str, counter)
